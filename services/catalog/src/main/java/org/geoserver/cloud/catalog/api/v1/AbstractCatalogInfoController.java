...
import org.geoserver.cloud.catalog.modelmapper.CatalogInfoMapper;
...
public abstract class AbstractCatalogInfoController<T extends CatalogInfo> {
    ...
    protected @Autowired CatalogInfoMapper mapper;

    protected abstract Class<T> getInfoType();
    protected abstract Function<T, Object> getMapperFunction();
    ...
    protected <S extends T> Class<S> getInfoType(@Nullable ClassMappings subType) {
        ...
        if (!getInfoType().isAssignableFrom(interf)) {
            throw new ResponseStatusException(BAD_REQUEST, String
                    .format("Invalid subtype of %s: %s", getInfoType().getSimpleName(), subType));
        }
        return interf;
    }
    ...
    public Mono<T> create(@RequestBody(required = true) T info) {
        return service.create(info, getInfoType()).switchIfEmpty(
                notFound("%s '%s' does not exist", getInfoType().getSimpleName(), info.getId()));
    }

    @PutMapping(path = "", consumes = "application/xml", produces = "application/xml")
    public Mono<T> update(@RequestBody(required = true) T info) {
        return service.update(info, getInfoType()).switchIfEmpty(
                notFound("%s '%s' does not exist", getInfoType().getSimpleName(), info.getId()));
    }
    ...
    public Mono<T> delete(@PathVariable(name = "id", required = true) String id) {
        return service.delete(id, getInfoType()).switchIfEmpty(
    ...
    public Flux<Object> findAll(
        ...
        return service.findAll(getInfoType(subType)).map(getMapperFunction()::apply);
    }
    
    @GetMapping(path = "/{id}")
    public Mono<T> findById(@PathVariable("id") String id,
            @RequestParam(name = "subtype", required = false) ClassMappings subType) {

        return service.findById(id, getInfoType(subType)).switchIfEmpty(
                notFound("%s '%s' does not exist", getInfoType().getSimpleName(), id));
    }

    @GetMapping(path = "/name/{name}")
    public Mono<T> findByName(@PathVariable(name = "name", required = true) String name,
            @RequestParam(name = "namespace", required = false) String namespace,
            @RequestParam(name = "subtype", required = false) ClassMappings subType) {

        Name qualifiedName = new NameImpl(namespace, name);
        return service.findByName(qualifiedName, getInfoType(subType)).switchIfEmpty(
                notFound("%s '%s:%s' does not exist", getInfoType(subType), namespace, name));
    }
    //
