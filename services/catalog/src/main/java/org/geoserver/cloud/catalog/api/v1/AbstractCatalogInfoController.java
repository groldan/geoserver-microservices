...
public abstract class AbstractCatalogInfoController<I extends CatalogInfo, DTO extends CatalogInfoDto> {
    ...
    public Mono<DTO> update(@RequestBody(required = true) DTO info) {
        return service.update(toInfo(info), getInfoType()).map(this::toDto).switchIfEmpty(
    ...
    public Mono<DTO> delete(@PathVariable(name = "id", required = true) String id) {
        return service.delete(id, getInfoType()).map(this::toDto).switchIfEmpty(
    ...
    @GetMapping(path = "/{id}")
    public Mono<DTO> findById(@PathVariable("id") String id,
            @RequestParam(name = "subtype", required = false) ClassMappings subType) {

        return service.findById(id, getInfoType(subType)).map(this::toDto).switchIfEmpty(
                notFound("%s '%s' does not exist", getInfoType().getSimpleName(), id));
    }

    @GetMapping(path = "/name/{name}")
    public Mono<DTO> findByName(@PathVariable(name = "name", required = true) String name,
            @RequestParam(name = "namespace", required = false) String namespace,
            @RequestParam(name = "subtype", required = false) ClassMappings subType) {

        Name qualifiedName = new NameImpl(namespace, name);
        return service.findByName(qualifiedName, getInfoType(subType)).map(this::toDto)
                .switchIfEmpty(notFound("%s '%s:%s' does not exist", getInfoType(subType),
                        namespace, name));
    }
    //
